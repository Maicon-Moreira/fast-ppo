...
from .utils import normalize, plot_training_info, save_info
...
def train(
    ...
    with ProcessPoolExecutor(max_workers=cpu_threads) as executor:
        for current_update in tqdm(range(total_updates), disable=True):
            ...
            mean_episode_reward = training_info["mean_episode_reward_hist"][-1]
            ...
            training_info["loss_hist"].append(loss.item())
            loss.backward()
            old_actor_critic.load_state_dict(actor_critic.state_dict())
            optimizer.step()
    ...
    print(f"Total time: {setup_time + training_time:.2f} seconds")
    print(
    ...
    return best_actor_critic, training_info
